        ...
        HttpServer.newServer(8080).start((req, resp) -> {
            ...
            switch (query) {
                case "register" -> {
                    ...
                    response = RxMongoDriver.getUsers()
                        ...
                        .flatMap(exists -> {
                                    ...
                                    if (exists) {
                                        return Observable.just("User with login " + login + " is already registered!");
                                    } else {
                                        ...
                                        return RxMongoDriver.addUser(user).map(success -> user.toString() + " successfully registered.");
                                    }
                                }
                        );
                }
                case "add" -> {
                    response = RxMongoDriver.getProducts()
                        .exists(product -> product.getName().equals(parameters.get("name").get(0)))
                        .flatMap(exists -> {
                                    String name = parameters.get("name").get(0);
                                    if (exists) {
                                        return Observable.just("Product with name " + name + " is already added!");
                                    } else {
                                        String currencyStr = parameters.get("currency").get(0);
                                        try {
                                            ...
                                            return RxMongoDriver.addProduct(product).map(success -> product.toString() + " successfully added.");
                    ...
                    response = RxMongoDriver.getUsers()
                            ...
                            .flatMap(user -> {
                                        if (user.getLogin().isEmpty()) {
                                            ...
                                            return RxMongoDriver.getProducts().map(product -> Product.pretty(product, currency));
                                        }
                                    }
                            );
                }
                case "drop" -> {
                    response = RxMongoDriver.drop().map(success -> "Successfully dropped");
                }
                default -> {
                    response = null;
                }
