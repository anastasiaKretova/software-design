...
public class MannysConverterAPITest {
    ...
    public void testUSDtoXCD() throws IOException {
        ...
        double expected = 2.7;
        double actual = 0;
        actual = MannysConverterAPI.rate(U_S_DOLLARS, eastCaribDollars);
    ...
    public void testXCDtoUSD() throws IOException {
        ...
        double actual = MannysConverterAPI.rate(eastCaribDollars, U_S_DOLLARS);
        assertEquals(expected, actual, TEST_DELTA);
    }
    ...
    public void testRate() throws IOException {
        ...
        double dollarsToYenRate = MannysConverterAPI.rate(U_S_DOLLARS, yen);
        System.out.println(MannysConverterAPI.rate(U_S_DOLLARS, Currency.getInstance("RUB")));
        double yenToDollarsRate = MannysConverterAPI.rate(yen, U_S_DOLLARS);
        System.out.println("Dollars to yen rate is said to be " + dollarsToYenRate);
        System.out.println("Yen to dollars rate is said to be " + yenToDollarsRate);
        double expected = 1.0;
