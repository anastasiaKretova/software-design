...
public class StatsService {
    ...
    public String getStatsForEachDay() {
        updateDatabase();
        Map<String, List<Event>> eventsForEachDay = events.stream().filter(EntryEvent.class::isInstance).collect(Collectors.groupingBy(e -> {
    ...
    public String getAverageTime() {
        updateDatabase();
        List<ModifyEvent> entryEvents = events.stream().filter(ModifyEvent.class::isInstance).map(ModifyEvent.class::cast).collect(Collectors.toList());
        double time = 0;
        int visits = 0;
        ...
        return String.format("Average time based on %d visits: %f minutes", visits, visits == 0 ? 0 : time / visits);
    }
